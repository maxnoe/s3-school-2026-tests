...
\title[Testing \& Debugging]{Unit Testing \& Debugging}
\date[2026-01-16]{%
  \Large S\textsuperscript{3} School – 2026-01-15
}
\institute[CTAO ERIC]{%
  Cherenkov Telescope Array Observatory ERIC\\[\baselineskip]%
  \includegraphics[height=0.75cm]{./build/license.pdf}%
}

\newcommand\good{\textcolor{green!70!black}{\faPlus}}
\newcommand\bad{\textcolor{red!80!black}{\faMinus}}
...
\begin{frame}[c, fragile]{Example python code}
  ...
  \inputcode[title=\texttt{examples/step1/fibonacci.py}]{python}{examples/step1/fibonacci.py}
\end{frame}
...
\begin{frame}[c, fragile]{Project Integration (without pixi)}
  \begin{itemize}
    \item Add pytest to the optional dependencies of your project:
      \begin{code}[title=\texttt{pyproject.toml}]{toml}
        [project.optional-dependencies]
        test = [
          "pytest",
        ]
      \end{code}
    \item This enables adding an \textit{extra} \enquote{test} to the installation command:
      \begin{code}{bash}
...
\begin{frame}[c, fragile]{Project integration (with pixi)}
  \begin{itemize}
    \item Add a feature \enquote{test} that depends on pytest:
      \begin{code}[title=\texttt{pyproject.toml}]{toml}
        [tool.pixi.feature.test.dependencies]
        pytest = "9.*"
      \end{code}
    \item Add an environment using this feature, e.g.:
      \begin{code}[title=\texttt{pyproject.toml}]{toml}
        [tool.pixi.environments]
        default = { solve-group = "default" }
        test = { features = ["test"], solve-group = "default" }
      \end{code}
    \item You can now run:
      \begin{code}{bash}
...
\begin{frame}[c, fragile]{Project integration (with pixi)}
  \begin{itemize}
    \item You could also add a task:
      \begin{code}[title=\texttt{pyproject.toml}]{toml}
...
\begin{frame}[c, fragile]{First Unit Test}
  \inputcode[title=\texttt{examples/step1/test\_fibonacci1.py}]{python}{examples/step1/test_fibonacci1.py}

  \inputcode[title=\texttt{pixi run pytest -sv}]<fontsize=\scriptsize>{text}{./examples/pytest_output.txt}
\end{frame}
...
\begin{frame}[c]{Testing Exceptions}
  ...
  \inputcode[title=\texttt{examples/step2/fibonacci.py}]<lastline=4>{python}{examples/step2/fibonacci.py}
  \inputcode[title=\texttt{examples/step2/test\_exception.py}]{python}{examples/step2/test_exception.py}
...
\begin{frame}[c, fragile]{Careful with floating point numbers}
  \inputcode[title={Naive, this fails}]<firstline=3, lastline=4>{python}{examples/floating/test_floating.py}
  \inputcode[title={Correct approach, using \texttt{pytest.approx}}]<firstline=6>{python}{examples/floating/test_floating.py}

  See \url{https://0.30000000000000004.com/}
\end{frame}

\begin{frame}[c, fragile]{Using numpy testing utitlities}
  \inputcode[title={Using numpy}]{python}{examples/floating/test_numpy.py}
  See \url{https://numpy.org/doc/stable/reference/routines.testing.html}
\end{frame}

\begin{frame}[c, fragile]{Using astropy quantity support}
  \inputcode[title={Using astropy units}]{python}{examples/floating/test_astropy.py}
\end{frame}
...
\begin{frame}[c]{Fixtures}
  ...
  \end{itemize}
  \inputcode{python}{examples/fixtures/test_fixtures.py}
\end{frame}
...
\begin{frame}[c]{Fixtures provided by pytest}
  ...
  \end{itemize}
  More at \url{https://docs.pytest.org/en/latest/fixture.html}
\end{frame}

\begin{frame}[c]{capsys – Fixture for testing the standard streams}
  \inputcode{python}{examples/fixtures/test_capsys.py}
\end{frame}

\begin{frame}[c]{caplog – Fixture for testing logging}
  \inputcode{python}{examples/fixtures/test_logging.py}
\end{frame}
...
\begin{frame}[c]{Temporary paths}
  \inputcode{python}{examples/fixtures/test_tmp_paths.py}
...
\begin{frame}[c]{Parametrized Tests and Fixtures}
  ...
  \inputcode[title={A parametrized test}]{python}{./examples/step3/test_parametrized.py}
\end{frame}
  ...
  Some tests can only be run under specific conditions
    \begin{itemize}
      \item Tests for features requiring optional dependencies
        \inputcode[title={This test is skipped when numpy is not available}]<firstline=5, lastline=7>{python}{examples/test_skipped.py}
      \item Tests for specific operating systems or versions
        \inputcode[title={This test is only executed on Windows}]<firstline=9>{python}{examples/test_skipped.py}
    \end{itemize}
\end{frame}
...
\begin{frame}[c, fragile]{Expected failures}
  ...
  \begin{itemize}
    ...
    \item Setting the strict option will result in failure if a test unexpectedly suceeds
      \begin{code}[title=pyproject.toml]{toml}
        [tool.pytest]
        xfail_strict = true
      \end{code}
  \end{itemize}

  \inputcode[title={This test is expected to fail}]{python}{examples/test_xfail.py}
\end{frame}
...
\begin{frame}[c, fragile]{Choosing which tests to run – Using markers}
  \begin{itemize}
    ...
    \item Add the marker to a test
      \inputcode<firstline=4, lastline=7>{python}{examples/test_marks.py}
...
\begin{frame}[c, fragile]{Create a coverage report}
  \begin{itemize}
    \item Needs the \texttt{pytest-cov} plugin, add to your test dependencies next to \texttt{pytest}
...
\begin{frame}[c]{Mocking / Monkeypatching}
  \inputcode{python}{examples/test_monkeypatching.py}
\end{frame}
...
\begin{frame}[c, fragile]{Doctests}
  \begin{itemize}
    \item Examples are an important part of every documentations
      \inputcode<firstline=1, lastline=8>{python}{fibonacci/src/fibonacci/__init__.py}
...
\section{Debugging}
\headlineframe{Debugging}

\begin{frame}[c, fragile]{Debugging approaches: print}
  Most common for beginners and even later: a generous seasoning of \texttt{print}s 
  \begin{itemize}
    \item[\good] Simple 
    \item[\bad] You need to modify the code
    \item[\bad] Hard to track which output comes from which part of the code\\
      We've all written something like this at some point:
        \begin{code}{python}
          ...
          print("AFTER foo")
        \end{code}
    \item[\bad] You need to remove all prints again when you are done
  \end{itemize}
\end{frame}

\begin{frame}[c, fragile]{Debugging approaches: logging}
  The logging module is part of the standard library 
  \begin{itemize}
    \item[\good] Simple 
    \item[\good] Useful also in production
    \item[\good] Decide on the level of different messages
    \item[\good] Configurable, what level you want to see from which logger
    \item[\good] Includes information about file, function, line of code that created the message
    \item[\bad] Code must be modified, messages pre-defined
  \end{itemize}
...
\begin{frame}[c, fragile]{Debugging approaches: running the debugger}
  \begin{code}{bash}
    $ python -m pdb script.py
  \end{code}
  \begin{itemize}
    \item You will enter the debugger and can execute code statement by statement
    \item You can also just run the code until it reaches a \texttt{breakpoint()} or an exception is raised
    \item You can inspect code, local variables, etc.
  \end{itemize}
\end{frame}

\begin{frame}[c, fragile]{Debugging as part of tests}
  \begin{itemize}
    \item Unit tests can be very useful for debugging

    \item E.\,g.\ Write a new test that fails because of the bug → investigate → make it pass
    ...
    \item You can also insert manual breakpoints
      \begin{code}{python}
        data = get_data()
    ...
    \item There are fancier debuggers than pdb, e.\,g.\ ipython's:
      \begin{code}{bash}
